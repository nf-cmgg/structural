...
def main(argv):
    parser = argparse.ArgumentParser(
        description=__doc__, prog="svtest medcov", formatter_class=argparse.RawDescriptionHelpFormatter
    )
...
def get_metrics(test_file, baseline_file, samples):
    ...
    metrics = {MEAN_KEY: float(sum(test_data)) / len(test_header)}
...
def get_baseline_metrics(metrics, baseline_file, test_data, samples):
    ...
    tu.test_iterable_sizes_equal(
        baseline_header, baseline_data, name_a="baseline file header", name_b="baseline file data row"
    )

...
def main(argv):
    parser = argparse.ArgumentParser(
        description=__doc__, prog="svtest merged-depth", formatter_class=argparse.RawDescriptionHelpFormatter
    )
    ...
    if (
        (bool(args.baseline_bed) ^ bool(args.test_hits))
        or (bool(args.baseline_bed) ^ bool(args.baseline_hits))
...
def get_metrics(ftest, fbase, contigs, type, test_hits_path, base_hits_path):
    ...
    metrics = {KEY_PREFIX + type + "_count": num_test_records}
...
def get_baseline_metrics(
    ...
    tu.test_sets_equal(
        test_header, base_header, item_str="header column", name_a="test file header", name_b="baseline file header"
    )

...
EXPECTED_COLUMNS = [
    ...
    "SR_posB_log_pval",
    "SR_sum_log_pval",
    "SR_posA_called_median",
    "SR_posB_called_median",
    "SR_sum_called_median",
    "SR_posA_bg_median",
    "SR_posB_bg_median",
    "SR_sum_bg_median",
    "SR_posA_bg_frac",
    ...
    "BAF_n_nonROH_cases",
    "BAF_n_samples",
    "BAF_mean_control_snps",
    "BAF_n_nonROH_controls",
    "BAF_n_controls",
]
...
def main(argv):
    parser = argparse.ArgumentParser(
        description=__doc__, prog="svtest metrics-file", formatter_class=argparse.RawDescriptionHelpFormatter
    )
...
def get_metrics(df, contigs, feature_cols, common):
    ...
    tu.test_sets_equal(
        df.columns, EXPECTED_COLUMNS, item_str="column", name_a="metric file header", name_b="expected columns"
    )
    ...
    if common:
        ...
        prefix = KEY_PREFIX
    metrics = {prefix + "num_records": df.size}

...
def main(argv):
    parser = argparse.ArgumentParser(
        description=__doc__, prog="svtest vcf", formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument("test_vcf", type=str)
    parser.add_argument("contig_list", type=str)
    parser.add_argument("sample_list", type=str)
    parser.add_argument("types", type=str, help="Comma-delimited list of variant types (case-sensitive)")
    parser.add_argument("metric_prefix", type=str)
    parser.add_argument("--baseline-vcf", type=str, help="Baseline vcf to provide evaluation metrics against")
    parser.add_argument(
        "--baseline-bed",
        type=str,
    ...
    parser.add_argument("--padding", type=int, default=50, help="Interval padding for validation metrics [50]")
    parser.add_argument(
        "--max-warnings", type=int, default=50, help="Maximum number of records to print warnings for [50]"
    )
    parser.add_argument("--fp-file", type=str, default=None, help="Write false positives to file")
    parser.add_argument("--fn-file", type=str, default=None, help="Write false negatives to file")
    parser.add_argument("--fp-pass-file", type=str, default=None, help="Write PASS false positives to file")
    parser.add_argument("--fn-pass-file", type=str, default=None, help="Write PASS false negatives to file")
    ...
    if (args.baseline_vcf is None and args.baseline_bed is None) and (
        args.fp_file is not None or args.fn_file is not None
    ):
...
def get_metrics(
    ftest, fbase_vcf, fbase_bed, contigs, variant_types, min_ro, padding, samples, metric_prefix, max_warnings
):
    ...
    if fbase_vcf is not None:
        ...
        test_tree = iu.create_trees_from_records(test_records, variant_types, contigs)
        test_pass_tree = iu.create_trees_from_records(pass_records, variant_types, contigs)
        base_tree = iu.create_trees_from_records(base_records, variant_types, contigs)
        base_pass_records = [
            r for r in base_records if ("PASS" in r.filter or len(set(r.filter) - pass_filter_set) == 0)
        ]
        base_pass_tree = iu.create_trees_from_records(base_pass_records, variant_types, contigs)
    elif fbase_bed is not None:
        base_records = parse_bed_file(fbase_bed)
        test_tree = iu.create_trees_from_records(test_records, variant_types, contigs)
        test_pass_tree = iu.create_trees_from_records(pass_records, variant_types, contigs)
        base_tree = iu.create_trees_from_bed_records(base_records, variant_types, contigs)
        base_pass_tree = None
    else:
        test_tree = None
        test_pass_tree = None
...
def add_evaluation_metrics(
    metrics, test_tree, base_tree, variant_types, min_ro, padding, metric_prefix, metric_suffix=""
):
    tp_test = {}
    fp_test = {}
    fp_intervals_test = {}
...
def parse_bed_file(path):
    ...
    return df[
        [BED_FILE_HEADER_CHAR + BED_FILE_CHROM_COL, BED_FILE_START_COL, BED_FILE_END_COL, BED_FILE_SVTYPE_COL]
    ].values

...
process VCFANNO {
    ...
    input:
    tuple(meta: Map, vcf: Path, tbi: Path, specific_resources: Path)
    toml: Path
    lua: Path
    resources: Path
    ...
    versions_vcfanno = tuple("${task.process}", 'vcfanno', eval("vcfanno 2>&1 | sed -n 's/.*version \\([0-9.]\\+\\).*/\\1/p'"))

    topic:
    tuple("${task.process}", 'vcfanno', eval("vcfanno 2>&1 | sed -n 's/.*version \\([0-9.]\\+\\).*/\\1/p'")) >> 'versions'

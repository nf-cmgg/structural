...
workflow VCF_ANNOTATE_VEP_ANNOTSV_VCFANNO {
    ...
    ENSEMBLVEP_VEP.out.vcf.view{ "vep vcf: ${it}" }
        .join(TABIX_VEP.out.tbi.view{ "vep tbi: ${it}" }, failOnDuplicate:true, failOnMismatch:true)
        .join(ch_annotsv_output.view { "annotsv: ${it}" }, failOnDuplicate:true, failOnMismatch:true)
        .view { "joined: ${it}" }
